...
import static com.ok.mexico.MxcCheckResult.RESULT_EMULATOR;
import static com.ok.mexico.MxcCheckResult.RESULT_MAYBE_EMULATOR;
import static com.ok.mexico.MxcCheckResult.RESULT_UNKNOWN;

public class MxcEmulatorCheckUtil {
    private MxcEmulatorCheckUtil() {

    }

    private static class SingletonHolder {
        private static final MxcEmulatorCheckUtil INSTANCE = new MxcEmulatorCheckUtil();
    }

    public static final MxcEmulatorCheckUtil getSingleInstance() {
        return SingletonHolder.INSTANCE;
    }

    public boolean readSysProperty(Context context, MxcEmulatorCheckCallback callback) {
        if (context == null)
        ...
        //检测硬件名称
        MxcCheckResult hardwareResult = checkFeaturesByHardware();
        ...
        //检测渠道
        MxcCheckResult flavorResult = checkFeaturesByFlavor();
        ...
        //检测设备型号
        MxcCheckResult modelResult = checkFeaturesByModel();
        ...
        //检测硬件制造商
        MxcCheckResult manufacturerResult = checkFeaturesByManufacturer();
        ...
        //检测主板名称
        MxcCheckResult boardResult = checkFeaturesByBoard();
        ...
        //检测主板平台
        MxcCheckResult platformResult = checkFeaturesByPlatform();
        ...
        //检测基带信息
        MxcCheckResult baseBandResult = checkFeaturesByBaseBand();
        ...
        //检测进程组信息
        MxcCheckResult cgroupResult = checkFeaturesByCgroup();
    ...
    private String getProperty(String propName) {
        String property = MxcCommandUtil.getSingleInstance().getProperty(propName);
     ...
     */
    private MxcCheckResult checkFeaturesByHardware() {
        String hardware = getProperty("ro.hardware");
        if (null == hardware) return new MxcCheckResult(RESULT_MAYBE_EMULATOR, null);
        int result;
        ...
        return new MxcCheckResult(result, hardware);
    }
     ...
     * @return 0表示可能是模拟器，1表示模拟器，2表示可能是真机
     */
    private MxcCheckResult checkFeaturesByFlavor() {
        String flavor = getProperty("ro.build.flavor");
        if (null == flavor) return new MxcCheckResult(RESULT_MAYBE_EMULATOR, null);
        int result;
        ...
        return new MxcCheckResult(result, flavor);
    }
     ...
     * @return 0表示可能是模拟器，1表示模拟器，2表示可能是真机
     */
    private MxcCheckResult checkFeaturesByModel() {
        String model = getProperty("ro.product.model");
        if (null == model) return new MxcCheckResult(RESULT_MAYBE_EMULATOR, null);
        int result;
        ...
        return new MxcCheckResult(result, model);
    }
     ...
     * @return 0表示可能是模拟器，1表示模拟器，2表示可能是真机
     */
    private MxcCheckResult checkFeaturesByManufacturer() {
        String manufacturer = getProperty("ro.product.manufacturer");
        if (null == manufacturer) return new MxcCheckResult(RESULT_MAYBE_EMULATOR, null);
        int result;
        ...
        return new MxcCheckResult(result, manufacturer);
    }
     ...
     * @return 0表示可能是模拟器，1表示模拟器，2表示可能是真机
     */
    private MxcCheckResult checkFeaturesByBoard() {
        String board = getProperty("ro.product.board");
        if (null == board) return new MxcCheckResult(RESULT_MAYBE_EMULATOR, null);
        int result;
        ...
        return new MxcCheckResult(result, board);
    }
     ...
     * @return 0表示可能是模拟器，1表示模拟器，2表示可能是真机
     */
    private MxcCheckResult checkFeaturesByPlatform() {
        String platform = getProperty("ro.board.platform");
        if (null == platform) return new MxcCheckResult(RESULT_MAYBE_EMULATOR, null);
        int result;
        String tempValue = platform.toLowerCase();
        if (tempValue.contains("android")) result = RESULT_EMULATOR;
        else result = RESULT_UNKNOWN;
        return new MxcCheckResult(result, platform);
    }
     ...
     * @return 0表示可能是模拟器，1表示模拟器，2表示可能是真机
     */
    private MxcCheckResult checkFeaturesByBaseBand() {
        String baseBandVersion = getProperty("gsm.version.baseband");
        if (null == baseBandVersion) return new MxcCheckResult(RESULT_MAYBE_EMULATOR, null);
        int result;
        if (baseBandVersion.contains("1.0.0.0")) result = RESULT_EMULATOR;
        else result = RESULT_UNKNOWN;
        return new MxcCheckResult(result, baseBandVersion);
    }
    ...
    private int getUserAppNumber() {
        String userApps = MxcCommandUtil.getSingleInstance().exec("pm list package -3");
     ...
     * 特征参数-进程组信息
     */
    private MxcCheckResult checkFeaturesByCgroup() {
        String filter = MxcCommandUtil.getSingleInstance().exec("cat /proc/self/cgroup");
        if (null == filter) return new MxcCheckResult(RESULT_MAYBE_EMULATOR, null);
        return new MxcCheckResult(RESULT_UNKNOWN, filter);
    }
}
